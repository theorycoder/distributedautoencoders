...
def _create_or_extend_private_grad(param: torch.Tensor, summed_clipped_grad: torch.Tensor, accumulate_private_grad = True) -> None:
    ...
    if hasattr(param, "private_grad"):
      if accumulate_private_grad == True:
        ...
        if param.noise == 0:
            #raise ValueError(f"param.noise must be positive, but got {param.noise}")
            param.private_grad = summed_clipped_grad.detach()
        else:
            #param.private_grad = summed_clipped_grad.detach() + torch.normal(mean=0, std=param.noise,size=param.size(), device=param.device, dtype=param.dtype) #Gaussian noise
            param.private_grad = summed_clipped_grad.detach() + torch.distributions.Laplace(loc=0.0, scale=param.noise).sample(param.size()).to(param.device, param.dtype) #Laplace noise mean=loc
