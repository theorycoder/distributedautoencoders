#use script $ for i in {1..14}; do python3 autoencoder2.py; done
#two encoders, 1 decoder
# this program uses the dpsg-optimizer library available at https://github.com/thecml/dpsgd-optimizer
...
if dpsgd:
    ...
    sen_noise_sig = 5 #sensor noise s.d.
    #if app_sen_noise==1:
...
print('dpsgd: ',dpsgd)
print('use_custom_loss: ',use_custom_loss)
...
class DA(tf.Module):
  ...
  def __call__(self, x):
    ...
    decoded = self.decoder(tf.concat([encoded1, encoded2], 1))
    
    return decoded
    

    
def my_loss(W_dec, enc1_out, enc2_out, y_true, y_pred): #custom loss has to be a differential function
	...
	for i in range(n): 
		...
		a=np.tensordot(enc2_out, W_dec[l:n,i], axes=1) 
		b=tf.math.square(np.tensordot(enc2_out, W_dec[l:n,i], axes=1))
		if app_FM_DP:
	...
	#print('val: ', val)
	#val2=tf.reduce_sum(sum1+sum2) 
	
	return (sum1+sum2) #summation over j
	
		    

model = DA(name='sequential_model')
...
def compute_loss(model, x, x_hat):
	...
	x_pred = model(x)
	if use_custom_loss==1:
		loss = my_loss(W_dec, enc1_out, enc2_out, y_true=x_hat, y_pred=x_pred)
	else:
...
def get_grad(model, x, y):
    with tf.GradientTape() as tape:
        ...
        '''
        grad0=gradients[0] #gradients[0] is immutable, it is a tf.Tensor object, not tf.Variable, so a value cannot be assigned to it
        grad0=np.asarray(grad0).astype('float32')
        #print("grad0: ", grad0)
        '''
        ...
        with tf.compat.v1.Session() as sess:  
        	...
        	sess.run(init)
        	#tfc.compat.v1.assign(npc).eval()
        	tf.compat.v1.assign(gradients[0], grad0)
        '''
...
if app_FM_DP==1 and use_custom_loss==0:
...
for epoch in range(10):
	...
	#print(verbose.format(epoch, tf.divide(train_acc, n_train), tf.divide(test_acc, n_test)))
	
	print('\n epoch: ', epoch)                
	
'''	
autoencoder_out = model(X_hat)
autoencoder_out = np.asarray(autoencoder_out, dtype="object")
#print('autoencoder_out: ', autoencoder_out)
for i in range(0, len(autoencoder_out[:,0])):
    for j in range(0, len(autoencoder_out[0,:])):
    	if autoencoder_out[i,j]>=0.5:
	   		X_pred[i,j]=1
'''	   		
...
if dpsgd==1:
    filename="dpsgdaccuracy"+"_"+str(I)+"_"+str(use_custom_loss)+"_"+str(app_sen_noise)+"_"+str(sen_noise_sig)+".txt" 
elif app_sen_noise==1:
    filename="FMaccuracy_noisyInp"+"_"+str(I)+"_"+str(use_bm)+"_"+str(sen_noise_sig)+".txt" #noisyInpB is when noise2 scale sigma=1 instead of 5
